...
$nPageSize = ($_GET["object_count"])?ObjectCount($_GET["object_count"]):"10";
	
$arNavParams = array(
	"nPageSize"=>$nPageSize,
	//"bDescPageNumbering" => 'Y',
	"bShowAll" => false,
	//"iNumPage" => 2,
);
...
/*************************************************************************
			Work with cache
*************************************************************************/
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation))
{
	...
	if(is_array($arResult["SECTION"]))
	{
		...
		$pieces = explode("/", $cur_page);
		//echo "<pre>", print_r(array_diff($pieces, array(''))), "</pre>";
		$pieces = array_diff($pieces, array(''));
		$cnt_cur = count($pieces);
		//echo $cnt_cur;
		if($cnt_cur==3)
			$arFilter["<="."DEPTH_LEVEL"]=2;			
		if($cnt_cur==4)
			$arFilter["<="."DEPTH_LEVEL"]=3;
	...
	$rsSections = CIBlockSection::GetList($arSort, $arFilter, $arParams["COUNT_ELEMENTS"], $arSelect, $arNavParams);
	//$rsSections->NavStart(5);
	$rsSections->SetUrlTemplates("", $arParams["SECTION_URL"]);
	...
	$arParams["PAGER_TEMPLATE"] = 'mt_page';
